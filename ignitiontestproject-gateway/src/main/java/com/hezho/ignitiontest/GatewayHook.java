package com.hezho.ignitiontest;

import java.sql.SQLException;
import java.util.Collections;
import java.util.List;

import com.hezho.ignitiontest.records.HCSettingsTESTRecord;
import com.inductiveautomation.ignition.common.BundleUtil;
import com.inductiveautomation.ignition.common.licensing.LicenseState;
import com.inductiveautomation.ignition.common.util.LogUtil;
import com.inductiveautomation.ignition.common.util.LoggerEx;
import com.hezho.ignitiontest.web.HCSettingsPage;
...
public class GatewayHook extends AbstractGatewayModuleHook {
     ...
     * HomeTESTConnectStatusRoutes.java, and the model and view will be in our javascript folder. The status panel is optional
...
//            "HomeTESTConnect.nav.status.header") {
//
//        @Override
//        public WebMarkupContainer getPanel(String panelId) {
//            // We've set  GatewayHook.getMountPathAlias() to return hce, so we need to use that alias here.
//            return new BasicReactPanel(panelId, "/main/res/hce/js/HomeTESTConnectstatus.js", "HomeTESTConnectstatus");
//        }
//
//        @Override
...
//    };
    ...
    public static final ConfigCategory CONFIG_CATEGORY =
            new ConfigCategory("HomeTESTConnect", "HomeTESTConnect.nav.header", 700);
     ...
     * with the right-hand value returned from {@link ConfigPanel#getMenuLocation}. In this case name("HomeTESTConnect")
    ...
    public static final IConfigTab HCE_CONFIG_ENTRY = DefaultConfigTab.builder()
            ...
            .name("HomeTESTConnect")
            .i18n("HomeTESTConnect.nav.settings.title")
    ...
    public void setup(GatewayContext gatewayContext) {
        ...
        log.debug("Beginning setup of HomeTESTConnect Module");

        // Register GatewayHook.properties by registering the GatewayHook.class with BundleUtils
        BundleUtil.get().addBundle("HomeTESTConnect", getClass(), "HomeTESTConnect");
        ...
        HCSettingsTESTRecord theOneRecord = context.getLocalPersistenceInterface().find(HCSettingsTESTRecord.META, 0L);
        ...
        HCSettingsTESTRecord.META.addRecordListener(new IRecordListener<HCSettingsTESTRecord>() {
            @Override
            public void recordUpdated(HCSettingsTESTRecord hcSettingsTESTRecord) {
                log.info("recordUpdated()");
            }

            @Override
            public void recordAdded(HCSettingsTESTRecord hcSettingsTESTRecord) {
    ...
    private void verifySchema(GatewayContext context) {
        try {
            context.getSchemaUpdater().updatePersistentRecords(HCSettingsTESTRecord.META);
        } catch (SQLException e) {
            log.error("Error verifying persistent record schemas for HomeTESTConnect records.", e);
        }
    }

    public void maybeCreateHCSettings(GatewayContext context) {
        log.trace("Attempting to create HomeTESTConnect Settings Record");
        try {
            HCSettingsTESTRecord settingsRecord = context.getLocalPersistenceInterface().createNew(HCSettingsTESTRecord.META);
            settingsRecord.setId(0L);
            settingsRecord.setHCIPAddress("192.168.1.99");
            settingsRecord.setHCHubName("HomeTESTConnect Hub");
        ...
        log.trace("HomeTESTConnect Settings Record Established");
    }
    ...
    public void shutdown() {
        /* remove our bundle */
        BundleUtil.get().removeBundle("HomeTESTConnect");
    }
