package com.hezho.ignitiontest.records;
...
/**
 * Filename: HCSettingsTESTRecord
 * Author: Perry Arellano-Jones
 * Created on: 6/8/15
 * Project: home-connect-example
 */
public class HCSettingsTESTRecord extends PersistentRecord {

    public static final RecordMeta<HCSettingsTESTRecord> META = new RecordMeta<HCSettingsTESTRecord>(
            HCSettingsTESTRecord.class, "HCSettingsTESTRecordT").setNounKey("HCSettingsTESTRecord.Noun").setNounPluralKey(
            "HCSettingsTESTRecord.Noun.Plural");

    public static final IdentityField Id = new IdentityField(META);

    //Hub Settings -- for our imaginary IoT controller device
    public static final StringField HCHubName = new StringField(META, "HomeTESTConnectHubName", SFieldFlags.SMANDATORY);
    ...
    // create categories for our record entries, getting titles from the HCSettingsTESTRecord.properties, and
    // ordering through integer ranking
    static final Category HubConfiguration = new Category("HCSettingsTESTRecord.Category.Configuration", 1000).include(HCHubName, HCIPAddress);
    static final Category Security = new Category("HCSettingsTESTRecord.Category.Security", 1001).include(BroadcastSSID, HCDeviceCount, AllowInterop);
    static final Category Power = new Category("HCSettingsTESTRecord.Category.Power", 1001).include(HCPowerOutput);
